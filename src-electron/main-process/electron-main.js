...
ipcMain.on('cancelTransaction', async (event, data) => {
  ...
  try {
    ...
    let resultCancel = await App.cancelTransaction(event, data)
...
ipcMain.on('setContext', async (event, data) => {
  ...
  try {
    log.info(tag,"checkpoint0")
    let setContextResult = await App.setContext(event, data)
...
ipcMain.on('onPairKeepKey', async (event, data) => {
  ...
  try {
    log.info(tag,"checkpoint0")
    let pairKeepKeyResult = await App.onPairKeepKey(event, data)
...
ipcMain.on('onAttemptCreateUsername', async (event, data) => {
  ...
  try {
    let createUserResult = await App.onAttemptCreateUsername(event, data)
...
ipcMain.on('continueSetup', async (event, data) => {
  ...
  try {
    ...
    if(resultSetup && resultSetup.setup) {
...
ipcMain.on('getUsbDevices', async (event, data) => {
  ...
  try {
    let getUsbDevicesResult = await App.getUsbDevices(event, data)
...
ipcMain.on('startNetwork', async (event, data) => {
  ...
  try {
    ...
    //result
    let result = App.startNetwork(event, data)
    log.info(tag,"result",result)
...
ipcMain.on('attemptPair', async (event, data) => {
  ...
  try {
    log.info(tag,"data: ",data)
    let result = await App.attemptPair(event,data)
...
ipcMain.on('buildTransaction', async (event, data) => {
  ...
  try {
    ...
    let transferUnSigned = await App.buildTransaction(event, data)
...
ipcMain.on('onPairKeepkey', async (event, data) => {
  ...
  try {
    ...
    let result = await App.onPairKeepkey(data)
    log.info(tag," result: ",result)
